...
public class SSMRunConfigurationExtension extends RunConfigurationExtension implements PluginAware {
    ...
    private static final Logger LOG = Logger.getInstance(SSMRunConfigurationExtension.class);
    ...
    public <T extends RunConfigurationBase<?>> void updateJavaParameters(
        ...
        try {
            ...
            if (pluginDescriptor == null) {
                LOG.error("[SSM Graph Plugin] Plugin descriptor is not initialized yet.");
                return;
            }
            ...
            LOG.info("[SSM Graph Plugin] Added javaagent for run config '" + configuration.getName() + "': " + pluginPath);
        } catch (Exception e) {
            LOG.error("[SSM Graph Plugin] Failed to add agent for run config '" + configuration.getName() + "': " + e.getMessage());
        }
    }
