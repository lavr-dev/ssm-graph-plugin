...
class SSMGraphToolWindowFactory : ToolWindowFactory {
        ...
        val panel = panel {
            ...
            row {
                button("Sync Dot") {
                    ...
                    val document = FileDocumentManager.getInstance().getDocument(dotFile)
                    if (document == null) {
                        JOptionPane.showMessageDialog(
                            null,
                    ...
                    FileEditorManager.getInstance(project).getAllEditors(dotFile).forEach { editor ->
                        when (editor) {
