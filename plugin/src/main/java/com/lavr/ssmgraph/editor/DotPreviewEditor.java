...
//                            browser.openDevtools();
    ...
    private void updatePreview(VirtualFile file) {
        var encoded = ReadAction.computeBlocking(
                () -> Optional.ofNullable(FileDocumentManager.getInstance().getDocument(file))
                        .map(document -> document.getText().getBytes(StandardCharsets.UTF_8))
